...
// path ["PATH"]: (--path)  path where scanning will start
#define OPT_path 1
...
// minsize ["SIZE"]: (--minsize)  min size of total duplicated space to report
#define OPT_minsize 12

// pathsep ["CHAR"]: (--pathsep)  change internal path separator to CHAR
#define OPT_pathsep 13

// cut ["PATHSEG"]: (--cut)  remove 'PATHSEG' from report paths
#define OPT_cut 14

// file ["PATH"]: (--file)  check this file
#define OPT_file 15

// exclude_path ["PATH"]: (--exclude-path)  ignore duplicates under
#define OPT_exclude_path 16

// link : (--link)  create symlinks for deleted files
#define OPT_link 17

// hardlink : (--hardlink)  create hard links for deleted files
...
// db ["PATH"]: (--db)  path to dupd database file
#define OPT_db 21
